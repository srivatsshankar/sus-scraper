class AboutScene extends Phaser.Scene {
    ...
    create() {
            ...
            const signContainer = this.add.container(400, 800);

            // Add sign image
            const sign = this.add.image(0, 0, 'sign').setOrigin(0.5);

            // Add multiple lines of credits text
            const creditsLines = [
                'CREDITS',
                '',
                'Game Development',
                ...
                'Partha Shankar',
                '',
                'Srivats Shankar (c) 2024'
            ];

            const textObjects = creditsLines.map((line, index) => {
                const yOffset = (index - creditsLines.length/2) * 30; // Spacing between lines
                return this.add.text(0, yOffset, line, {
                    fontFamily: '"Carrier Command", monospace',
                    fontSize: '16px',
                    fill: '#fff',
                    stroke: '#000',
            ...
            signContainer.add([sign, ...textObjects]);
